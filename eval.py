...
def load_model(sess, hps):
    ...
    '''if 'dataset=cifar10_model=cnn_lenet_small_method=robust_eps=0.007843_checkpoint' in hps.model_path:
        # mean, std = np.array([[[0.485, 0.456, 0.406]]]), 0.225
        mean, std = np.array([[[0.485, 0.456, 0.406]]]), 0.225
        param_file['weights_conv1'] = param_file['weights_conv1'] / std
        mean_flat = np.tile(mean, [4, 4, 1]).flatten()
        for i in range(param_file['biases_conv1'].shape[0]):  # for every bias
            conv1_flat = param_file['weights_conv1'][:, :, :, i].flatten()  # note: already divided by std
            param_file['biases_conv1'][i] = param_file['biases_conv1'][i] - mean_flat @ conv1_flat'''
...
parser.add_argument('--n_test_eval', type=int, default=1000, help='On how many examples to perform the evaluation.')
...
with tf.Session(graph=graph, config=config) as sess:
    with graph.as_default(), tf.device('/gpu:0'):
        if hps.p in [2, np.inf]: pgd_ae_tensor = ae.pgd_attack(x_tf, y_in, cleverhans_model, hps.p, eps, hps.pgd_n_iter)
    ...
    # ------------- Eval PGD -----------------------
    ### l1-pgd not supported at the moment
