...
class InfBall(DualObject):
    ...
    def bounds(self, network=None): 
        ...
        ###
        if self.q_norm == 'l1': l1 = nu_1.abs().sum(1)
        elif self.q_norm == 'l2': l1 = nu_1.norm(p=2, dim=1)
        elif self.q_norm == 'linf': l1, _ = nu_1.abs().max(dim=1)
        ###
    ...
    def objective(self, *nus): 
        ...
        ###
        if self.q_norm == 'l1': l1 = epsilon*nu.abs().sum(2)
        elif self.q_norm == 'l2': l1 = epsilon * nu.norm(p=2, dim=2)
        elif self.q_norm == 'linf':
          l1, _ = nu.abs().max(dim=2)
          l1 = epsilon * l1
        ###
